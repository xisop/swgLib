...
{
	std::size_t skmgSize;
	std::size_t total = readFormHeader(file, "SKMG", skmgSize);
	skmgSize += 8;
...
unsigned int skmg::readPSDT(std::istream& file, psdt& newPsdt)
{
	...
	std::size_t total = readFormHeader(file, "PSDT", psdtSize);
	psdtSize += 8;
...
{
	std::size_t trtsSize;
	std::size_t total = readFormHeader(file, "TRTS", trtsSize);
	trtsSize += 8;
...
{
	std::size_t bltsSize;
	std::size_t total = readFormHeader(file, "BLTS", bltsSize);
	bltsSize += 8;
...
{
	std::size_t bltSize;
	std::size_t total = readFormHeader(file, "BLT ", bltSize);
	bltSize += 8;
...
unsigned int skmg::readOZN(std::istream& file)
{
	std::string type;
	std::size_t oznSize;
	std::size_t total = readRecordHeader(file, type, oznSize);
	oznSize += 8;
	if (type != "OZN ")
	{
...
unsigned int skmg::readFOZC(std::istream& file)
{
	std::string type;
	std::size_t fozcSize;
	std::size_t total = readRecordHeader(file, type, fozcSize);
	fozcSize += 8;
	if (type != "FOZC")
	{
...
unsigned int skmg::readOZC(std::istream& file)
{
	std::string type;
	std::size_t ozcSize;
	std::size_t total = readRecordHeader(file, type, ozcSize);
	ozcSize += 8;
	if (type != "OZC ")
	{
...
unsigned int skmg::readZTO(std::istream& file)
{
	std::string type;
	std::size_t ztoSize;
	std::size_t total = readRecordHeader(file, type, ztoSize);
	ztoSize += 8;
	if (type != "ZTO ")
	{
...
unsigned int skmg::readSKTM(std::istream& file)
{
	std::string type;
	std::size_t sktmSize;
	std::size_t total = readRecordHeader(file, type, sktmSize);
	sktmSize += 8;
	if (type != "SKTM")
	{
...
unsigned int skmg::readTXCI(std::istream& file, psdt&/*newPsdt*/)
{
	std::string type;
	std::size_t txciSize;
	std::size_t total = readRecordHeader(file, type, txciSize);
	txciSize += 8;
	if (type != "TXCI")
	{
...
{
	std::size_t tcsfSize;
	std::size_t total = readFormHeader(file, "TCSF", tcsfSize);
	tcsfSize += 8;
...
{
	std::size_t primSize;
	std::size_t total = readFormHeader(file, "PRIM", primSize);
	primSize += 8;
...
unsigned int skmg::readTCSD(std::istream& file, psdt& newPsdt)
{
	std::string type;
	std::size_t tcsdSize;
	std::size_t total = readRecordHeader(file, type, tcsdSize);
	tcsdSize += 8;
	if (type != "TCSD")
	{
...
unsigned int skmg::readITL(std::istream& file, psdt& newPsdt)
{
	std::string type;
	std::size_t itlSize;
	std::size_t total = readRecordHeader(file, type, itlSize);
	itlSize += 8;
	if (type != "ITL ")
	{
...
unsigned int skmg::readOITL(std::istream& file, psdt& newPsdt)
{
	std::string type;
	std::size_t oitlSize;
	std::size_t total = readRecordHeader(file, type, oitlSize);
	oitlSize += 8;
	if (type != "OITL")
	{
...
{
	std::string type;
	std::size_t priminfoSize;
	std::size_t total = readRecordHeader(file, type, priminfoSize);
	priminfoSize += 8;
	if (type != "INFO")
	{
...
{
	std::string type;
	std::size_t bltinfoSize;
	std::size_t total = readRecordHeader(file, type, bltinfoSize);
	bltinfoSize += 8;
	if (type != "INFO")
	{
...
unsigned int skmg::readPOSN(std::istream& file)
{
	std::string type;
	std::size_t posnSize;
	std::size_t total = readRecordHeader(file, type, posnSize);
	posnSize += 8;
	if (type != "POSN")
	{
...
unsigned int skmg::readNORM(std::istream& file)
{
	std::string type;
	std::size_t normSize;
	std::size_t total = readRecordHeader(file, type, normSize);
	normSize += 8;
	if (type != "NORM")
	{
...
unsigned int skmg::readDOT3(std::istream& file)
{
	std::string type;
	std::size_t dot3Size;
	std::size_t total = readRecordHeader(file, type, dot3Size);
	dot3Size += 8;
	if (type != "DOT3")
	{
...
unsigned int skmg::readTWHD(std::istream& file)
{
	std::string type;
	std::size_t twhdSize;
	std::size_t total = readRecordHeader(file, type, twhdSize);
	twhdSize += 8;
	if (type != "TWHD")
	{
...
unsigned int skmg::readTWDT(std::istream& file)
{
	std::string type;
	std::size_t twdtSize;
	std::size_t total = readRecordHeader(file, type, twdtSize);
	twdtSize += 8;
	if (type != "TWDT")
	{
...
unsigned int skmg::readPIDX(std::istream& file, psdt& newPsdt)
{
	std::string type;
	std::size_t pidxSize;
	std::size_t total = readRecordHeader(file, type, pidxSize);
	pidxSize += 8;
	if (type != "PIDX")
	{
...
unsigned int skmg::readNIDX(std::istream& file, psdt& newPsdt)
{
	std::string type;
	std::size_t nidxSize;
	std::size_t total = readRecordHeader(file, type, nidxSize);
	nidxSize += 8;
	if (type != "NIDX")
	{
...
unsigned int skmg::readVDCL(std::istream& file, psdt&/*newPsdt*/)
{
	std::string type;
	std::size_t vdclSize;
	std::size_t total = readRecordHeader(file, type, vdclSize);
	vdclSize += 8;
	if (type != "VDCL")
	{
...
unsigned int skmg::readXFNM(std::istream& file)
{
	std::string type;
	std::size_t xfnmSize;
	std::size_t total = readRecordHeader(file, type, xfnmSize);
	xfnmSize += 8;
	if (type != "XFNM")
	{
...
unsigned int skmg::readBLTHPTS(std::istream& file)
{
	std::string type;
	std::size_t hptsSize;
	std::size_t total = readRecordHeader(file, type, hptsSize);
	hptsSize += 8;
	if (type != "HPTS")
	{
