...
{
	std::size_t dtiiSize;
	std::size_t total = readFormHeader(file, "DTII", dtiiSize);
	dtiiSize += 8;
...
unsigned int dtii::readCOLS(std::istream& file)
{
	std::size_t colsSize;
	std::string type;
	std::size_t total = readRecordHeader(file, type, colsSize);
	colsSize += 8;
	if (type != "COLS")
	{
...
unsigned int dtii::readTYPE(std::istream& file)
{
	std::size_t typeSize;
	std::string type;
	std::size_t total = readRecordHeader(file, type, typeSize);
	typeSize += 8;
	if (type != "TYPE")
	{
...
unsigned int dtii::readROWS(std::istream& file)
{
	std::size_t rowsSize;
	std::string type;
	std::size_t total = readRecordHeader(file, type, rowsSize);
	rowsSize += 8;
	if (type != "ROWS")
	{
