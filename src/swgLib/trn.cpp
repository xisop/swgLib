...
unsigned int trn::readTGEN(std::istream& file, const std::string& debugString)
{
	...
	std::size_t total = readFormHeader(file, "TGEN", tgenSize);
	tgenSize += 8;
...
unsigned int trn::readWMAP(std::istream& file, const std::string& debugString)
{
	...
	std::size_t total = readRecordHeader(file, type, wmapSize);
	wmapSize += 8;
	if (type != "WMAP")
	{
...
unsigned int trn::readSMAP(std::istream& file, const std::string& debugString)
{
	...
	std::size_t smapSize;
	std::string type;
	std::size_t total = readRecordHeader(file, type, smapSize);
	smapSize += 8;
	...
	sfam& newSFAM)
{
	...
	std::size_t total = readRecordHeader(file, type, sfamSize);
	sfamSize += 8;
	if (type != "SFAM")
	{
	...
	const std::string& debugString)
{
	...
	std::size_t total = readFormHeader(file, form, formSize, type);
	formSize += 8;
...
unsigned int trn::readSGRP(std::istream& file, const std::string& debugString)
{
	...
	std::size_t sgrpSize;
	std::size_t total = readFormHeader(file, "SGRP", sgrpSize);
	sgrpSize += 8;
...
unsigned int trn::readFGRP(std::istream& file, const std::string& debugString)
{
	...
	std::size_t fgrpSize;
	std::size_t total = readFormHeader(file, "FGRP", fgrpSize);
	fgrpSize += 8;
...
unsigned int trn::readRGRP(std::istream& file, const std::string& debugString)
{
	...
	std::size_t total = readFormHeader(file, form, rgrpSize, type);
	rgrpSize += 8;
...
unsigned int trn::readEFAM(std::istream& file, const std::string& debugString)
{
	...
	std::size_t efamSize;
	std::size_t total = readFormHeader(file, "EFAM", efamSize);
	efamSize += 8;
...
unsigned int trn::readEGRP(std::istream& file, const std::string& debugString)
{
	...
	std::size_t total = readFormHeader(file, form, egrpSize, type);
	egrpSize += 8;
...
unsigned int trn::readMFRC(std::istream& file, const std::string& debugString)
{
	...
	std::string form;
	std::string type;
	std::size_t total = readFormHeader(file, form, mfrcSize, type);
	mfrcSize += 8;
...
unsigned int trn::readMFAM(std::istream& file, const std::string& debugString)
{
	...
	std::size_t mfamSize;
	std::size_t total = readFormHeader(file, "MFAM", mfamSize);
	mfamSize += 8;
...
unsigned int trn::readMGRP(std::istream& file, const std::string& debugString)
{
	...
	std::size_t mgrpSize;
	std::size_t total = readFormHeader(file, "MGRP", mgrpSize);
	mgrpSize += 8;
...
unsigned int trn::readLYRS(std::istream& file, const std::string& debugString)
{
	...
	std::size_t lyrsSize;
	std::size_t total = readFormHeader(file, "LYRS", lyrsSize);
	lyrsSize += 8;
