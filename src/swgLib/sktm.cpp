...
{
	std::size_t sktmSize;
	std::size_t total = readFormHeader(file, "SKTM", sktmSize);
	sktmSize += 8;
...
unsigned int sktm::readRPRE(std::istream& file)
{
	std::string type;
	std::size_t rpreSize;
	std::size_t total = readRecordHeader(file, type, rpreSize);
	rpreSize += 8;
	if (type != "RPRE")
	{
...
unsigned int sktm::readRPST(std::istream& file)
{
	std::string type;
	std::size_t rpstSize;
	std::size_t total = readRecordHeader(file, type, rpstSize);
	rpstSize += 8;
	if (type != "RPST")
	{
...
unsigned int sktm::readBPTR(std::istream& file)
{
	std::string type;
	std::size_t bptrSize;
	std::size_t total = readRecordHeader(file, type, bptrSize);
	bptrSize += 8;
	if (type != "BPTR")
	{
...
unsigned int sktm::readBPRO(std::istream& file)
{
	std::string type;
	std::size_t bproSize;
	std::size_t total = readRecordHeader(file, type, bproSize);
	bproSize += 8;
	if (type != "BPRO")
	{
...
unsigned int sktm::readJROR(std::istream& file)
{
	std::string type;
	std::size_t jrorSize;
	std::size_t total = readRecordHeader(file, type, jrorSize);
	jrorSize += 8;
	if (type != "JROR")
	{
