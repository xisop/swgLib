...
bool treClass::readHeader(std::istream& file)
{
...
bool treClass::writeHeader(std::ostream& file)
{
...
void treClass::printHeader(std::ostream& os) const
{
	if ("5000" == version)
	{
		os << "File type: EERT\n"
			...
			<< "File Block Offset: " << fileOffset << "\n"
			<< "File Block Compression: " << fileCompression << "\n"
			<< "File Block Size: " << fileSize << "\n"
			<< "File Block Final Size: " << fileFinalSize << "\n"
			<< "Name Block Compression: " << nameCompression << "\n"
			<< "Name Block Size: " << nameSize << "\n"
			<< "Name Block Final Size: " << nameFinalSize << "\n";
	}
	else if ("6000" == version)
	{
		os << "File type: EERT\n"
...
bool treClass::readFileBlock(std::istream& file)
{
...
bool treClass::readNameBlock(std::istream& file)
{
	...
	{
		uint32_t charOffset = i->getNameOffset();
...
bool treClass::readMD5sums(std::istream& file)
{
...
std::stringstream*
treClass::saveRecordAsStream(const uint32_t& recordNum, bool verbose)
{
	...
	if (verbose)
	{
		std::cout << "Found record: " << fileRecordList[recordNum].getFileName()
			<< std::endl;
		fileRecordList[recordNum].print(std::cout);
	}
	...
	for (int i = 0; i < 16; ++i)
	{
		std::cout << std::hex << (uint32_t)mdArray[i];
	}
...
bool treClass::saveRecordAsFile(const uint32_t& recordNum)
{
	// Fail if record is out of range...
	if (recordNum >= fileRecordList.size())
	{
...
bool treClass::writeFileBlock(std::ostream& file)
{
	if (fileRecordList.empty())
	{
	...
	for (i = fileRecordList.begin(); i != fileRecordList.end(); ++i)
	{
		...
		totalNameBlockSize +=
			static_cast<uint32_t>(i->getFileName().size()) + 1;
	}
	...
	fileFinalSize =
		static_cast<uint32_t>(fileRecordList.size()) * treFileRecord::SIZE;
	...
	*/
	...
	numRecords = static_cast<uint32_t>(fileRecordList.size());
...
void treClass::setFileBlockCompression(const uint32_t& f)
{
	if ((0 == f) || (2 == f))
	{
		fileCompression = f;
...
void treClass::setNameBlockCompression(const uint32_t& f)
{
	if ((0 == f) || (2 == f))
	{
		nameCompression = f;
